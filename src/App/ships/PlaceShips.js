const isOccupied = (grid, row, col, ships, currentShip, rotated) => {
  let isTaken = false;
  if (rotated) {
...
export const placeShip = ({ grid, row, col, ships, currentShip, rotated }) => {
  if (isOccupied(grid, row, col, ships, currentShip, rotated)) {
    return null;
  } else {
    if (rotated) {
      if (row + ships[currentShip].size <= 11) {
